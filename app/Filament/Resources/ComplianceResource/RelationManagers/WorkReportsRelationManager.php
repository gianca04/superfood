                        ...
                        Tabs\Tab::make('Información general')
                            ...
                            ->schema([
                                ...
                                Forms\Components\Select::make('project_id')
                                    ...
                                    // 2. Bloqueamos para que no se pueda cambiar, pero que sea visible
                                    ->disabled()
                                    // 3. Importante: dehydrated() asegura que el valor se envíe al guardar aunque esté disabled
                                    ->dehydrated()
                                    ->options(function (callable $get) {
                                        // Cargamos las opciones para que el label (Nombre - Correlativo) se muestre correctamente
                                        return Project::query()
                                            ->select('id', 'name', 'quote_id')
                                            ->get()
                                            ->mapWithKeys(fn($project) => [
                                                $project->id => "{$project->name} - {$project->quote_id}"
                                            ])
                                            ->toArray();
                                    })
                                    ->searchable()
                                    ...
                                    // 4. MANTENEMOS EL BOTÓN DE VISUALIZACIÓN
                                    ->suffixAction(
                                        Forms\Components\Actions\Action::make('view_project_details')
                                            ->icon('heroicon-o-eye')
                                            ...
                                            ->modalSubmitAction(false)
                                            ->modalCancelActionLabel('Cerrar')
                                            // Solo visible si hay un proyecto seleccionado (que siempre lo habrá por el default)
