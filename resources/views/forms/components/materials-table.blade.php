...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div x-data="materialsTableComponent($wire.entangle('{{ $getStatePath() }}'), {{ $isDisabled ? 'true' : 'false' }})" wire:ignore>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-800">
                    <tr>
                        ...
                        @if (!$isDisabled)
                            <th scope="col"
                                class="px-4 py-3 text-center text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider w-24">
                                Acciones
                            </th>
                        @endif
                    </tr>
                ...
                <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                    <template x-for="(row, index) in rows" :key="index">
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                            <td class="px-4 py-3">
                                <input type="text" x-model="row.material" @input="updateState()"
                                    :disabled="isDisabled" placeholder="Ej: Cemento"
                            ...
                            <td class="px-4 py-3">
                                <input type="text" x-model="row.cantidad" @input="updateState()"
                                    :disabled="isDisabled" placeholder="Ej: 10"
